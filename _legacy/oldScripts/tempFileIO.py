...
Class to store and define ROIs.
    ...
    def __init__(self,genotype,gender,stimulus,celltype,cellnumber,saveDir='~',offset = 0.5):
        ...
        self.stimulusList = {'ascAmp':np.array([22.0,22.5,21.5,23.,21.,24.,20.,26.,18.]),
                             'ascAmpFlip':np.array([22.0,21.5,22.5,21.,23.,20.,24.,18.,26.]),
                             'descAmp':np.array([22.0,18.,26.,20.,24.,21.,23.,21.5,22.5]),
                             'descAmpFlip':np.array([22.0,26.,18.,24.,20.,23.,21.,22.5,21.5])}
        self.getStimulus()
    ...
    def interpFrameTemperature(self,frameTemp,newFrames):
        # in older versions of the DAQ sometimes not all frames were recognised
        # therefore we lineary fit them here.        
        if newFrames[-1] > frameTemp[-1,0]:
            frameTemp[:,0] = np.linspace(0,newFrames[-1],len(frameTemp))
    ...
    def verboseModeTXT(self,workingDir):
        sensorFpos   = self.getfPos('*.txt',workingDir)
        responseFpos = self.getfPos('*.txt',workingDir)
        data = self.readInDataTXT(responseFpos,sensorFpos) 
        return data

    def patchUpNikoData(self):
        #frames = np.empty((500,1))
        frames = np.arange(6000,6500).T
        emptyDFbyF = np.empty((500))
        temperature = np.empty((500))
        emptyDFbyF[:] = np.NAN
        temperature[:] = 22.0
        test = np.vstack((frames,temperature))
        test = np.vstack((test,emptyDFbyF)).T
        print test.shape, self.data.shape

        self.data = np.vstack((self.data,test))
